#!/usr/bin/env bash
# Standard WP test suite installer (wp-cli/scaffold template)
if [ $# -lt 3 ]; then
...
install_wp() {
    if [ -d $WP_CORE_DIR ]; then return 0; fi
    mkdir -p $WP_CORE_DIR
...
install_test_suite() {
    if [ -d $WP_TESTS_DIR ]; then return 0; fi
...
install_wp
install_test_suite
if [ "$SKIP_DB_CREATE" = "false" ]; then create_db; fi
